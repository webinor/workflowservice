...
class WorkflowValidationController extends Controller
{
    public function getDocumentsToValidateByRole(Request $request)
    {
      ...
      $documents_with_permissions =  $this->checkPermissions( $data , $request);
...
$filtered = collect($documents)->filter(function ($doc) use ($permissionsByDocId) {
    return isset($permissionsByDocId[$doc['id']]) 
        && $permissionsByDocId[$doc['id']]['permissions']['view'] === true;
})->values()->toArray();
    ...
    public function checkPermissions(array $rawDocuments , $request)
{
    ...
    $response = Http::withToken($request->bearerToken())
    ->acceptJson()->post(config('services.user_service.base_url') . '/permissions/check-batch-documents', $payload);
    ...
    function transformToPayload(array $raw, int $userId, array $actions = ['view', 'validate'])
{
    return [
        'userId' => $userId,
        'documents' => collect($raw['documents'] ?? [])->map(function ($doc) {
            return [
                'doc_id' => $doc['id'],
                'doctype_id' => $doc['document_type_id'],
