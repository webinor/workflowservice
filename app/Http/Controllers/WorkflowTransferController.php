    ...
    public function transferDocument(Request $request)
    {
        ...
        try {
            // 1️⃣ Récupérer l'instance du workflow du document
             $workflowInstance = WorkflowInstance::where('document_id', $validated['document_id'])->firstOrFail();

// 2️⃣ Récupérer l'étape en cours
  $step = WorkflowInstanceStep::where('workflow_instance_id', $workflowInstance->id)
    ...
    ->firstOrFail();
            ...
            $deptResponse = Http::acceptJson()->get(config("services.department_service.base_url")."/{$validated['transfer_to']}/hierarchie");

            if ($deptResponse->failed()) {
                throw new \Exception('Impossible de récupérer le responsable du département.');
            }

               $responsable = $deptResponse->json(); // ex: ['user_id' => 12]

            // 3️⃣ Appel au microservice user pour récupérer le role du responsable
              $roleResponse = Http::acceptJson()->get(config("services.user_service.base_url")."/roles/search?name={$responsable['position']['name']}");

            if ($roleResponse->failed()) {
                throw new \Exception('Impossible de récupérer le role du responsable.');
            }
